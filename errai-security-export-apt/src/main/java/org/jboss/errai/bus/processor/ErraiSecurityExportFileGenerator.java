...
public class ErraiSecurityExportFileGenerator extends AbstractProcessor {
  ...
  private TypeSpec generateExportFile(final TypeElement annotation, Set<? extends Element> elements) {
    return TypeSpec.classBuilder(buildExportFileNameForAnnotation(annotation))
            .addAnnotation(ErraiModuleExportFile.class)
  ...
  private String buildExportFileNameForAnnotation(final TypeElement annotation) {
    String annotationName = annotation.getQualifiedName().toString().replace(".", "_");
    return "ErraiModuleExportFile_" + annotationName;
  }

  private void saveExportFile(final TypeSpec exportFileTypeSpec) {
    try {
      JavaFile.builder(exportFilePackage(), exportFileTypeSpec).build().writeTo(processingEnv.getFiler());
    } catch (IOException e) {
      throw new RuntimeException("Error writing generated export file", e);
    }
  }

  private String exportFilePackage() {
    return ExportFilePackage.subPackage("bus");
  }
}